...
import { Color, FontWeight, Theme } from '@/constants/theme';
import { useCreateOpacity } from '@/hooks/theme';
...
const ButtonPressable = ({
  ...
  const titleColorStyle = useMemo(() => {
    const opacity =
      pressed && titleColorPressedOpacity ? titleColorPressedOpacity : titleColorOpacity;
    return colorStyle(titleColorActual, opacity);
  }, [colorStyle, pressed, titleColorActual, titleColorOpacity, titleColorPressedOpacity]);
  ...
  const backgroundColorStyle = useMemo(() => {
    if (disabled && backgroundColorDisabled) {
      return backgroundStyle(backgroundColorDisabled, backgroundColorDisabledOpacity);
    }
    if (pressed || selected) {
